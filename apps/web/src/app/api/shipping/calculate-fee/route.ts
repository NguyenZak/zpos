  ...
  try {
    ...
    if (!to_address) {
      return NextResponse.json(
        { success: false, error: "Thiếu địa chỉ giao hàng" },
        { status: 400 }
      );
    }

    const addressStr = typeof to_address === "string" ? to_address.toLowerCase() : JSON.stringify(to_address).toLowerCase();
    
    // Giả định địa chỉ gửi hàng là TP. Hồ Chí Minh
    const isInCity = addressStr.includes("hồ chí minh") || addressStr.includes("hcm") || addressStr.includes("quận") || addressStr.includes("tphcm");
