...
import ProductsManager from "@/app/app/products/page";
...
export default function CMSDashboard() {
  ...
  const [blogPosts, setBlogPosts] = useState<BlogPost[]>([
    {
      ...
      summary:
        "Chia sẻ 5 phương pháp tối ưu hóa dữ liệu kho, đồng bộ hóa hóa đơn tức thì và quản lý hiệu suất nhân viên để hạn chế gián đoạn thanh toán.",
    ...
    },
    {
      ...
      summary:
        "Đánh giá hiệu quả thực tế khi tích hợp Voice AI Assistant vào ZPOS giúp nhân viên giảm 50% thời gian nhập liệu hóa đơn.",
    ...
    },
    {
      ...
      summary:
        "Kiến trúc kỹ thuật đằng sau hệ thống quản lý kho đa chi nhánh (Multi-location) và cách thức vận hành đồng bộ không xung đột dữ liệu.",
  ...
  const [seoAnalysis, setSeoAnalysis] = useState({
    score: 82,
    feedback:
      "Bài viết có độ dài tốt và chứa từ khóa chính. Hãy bổ sung thêm alt text cho hình ảnh và một vài liên kết nội bộ.",
    ...
    toast.promise(new Promise((resolve) => setTimeout(resolve, 1500)), {
      ...
      success: () => {
        ...
        setSeoAnalysis({
          score: randomScore,
          feedback:
            "Quá xuất sắc! Bài viết chứa từ khóa mật độ lý tưởng (2.4%), thẻ tiêu đề chuẩn SEO H1/H2, mô tả tóm tắt hấp dẫn kích thích click.",
  ...
  const [mediaItems, setMediaItems] = useState<MediaItem[]>([
    ...
    {
      id: "img-2",
      name: "restaurant_ai_assistant.jpg",
      type: "image",
      size: "820 KB",
      url: "https://images.unsplash.com/photo-1531746790731-6c087fecd793?auto=format&fit=crop&w=600&q=80",
      uploadedAt: "2026-05-15 09:12",
    },
    {
      id: "img-3",
      name: "office_analytics_dashboard.jpg",
      type: "image",
      size: "1.2 MB",
      url: "https://images.unsplash.com/photo-1460925895917-afdab827c52f?auto=format&fit=crop&w=600&q=80",
      uploadedAt: "2026-05-14 17:05",
    },
    {
      id: "doc-1",
      name: "zpos_features_brochure.pdf",
      type: "document",
      size: "3.4 MB",
      url: "/docs/zpos_features_brochure.pdf",
      uploadedAt: "2026-05-10 11:30",
    },
    {
      id: "img-4",
      name: "barcode_scanner_setup.png",
      type: "image",
      size: "185 KB",
      url: "https://images.unsplash.com/photo-1607082348824-0a96f2a4b9da?auto=format&fit=crop&w=600&q=80",
      uploadedAt: "2026-05-08 10:15",
    },
  ]);
  ...
  const handleUpdateInquiryStatus = async (id: string, status: "new" | "contacted" | "completed") => {
    try {
      ...
      if (json.success && json.data) {
        ...
        toast.success(
          `Đã cập nhật trạng thái sang: ${status === "new" ? "Mới" : status === "contacted" ? "Đã liên hệ" : "Hoàn tất"}`,
        );
      } else {
  ...
  const [storeName, setStoreName] = useState("ZPOS Fashion");
  const [storeLogo, setStoreLogo] = useState("https://zpos.vn/logo.png");
  const [storeFavicon, setStoreFavicon] = useState("https://zpos.vn/favicon.ico");
  const [contactEmail, setContactEmail] = useState("support@zpos.vn");
  const [contactPhone, setContactPhone] = useState("0909 123 456");
  const [storeAddress, setStoreAddress] = useState("123 Nguyễn Văn Linh, Q.7, TP.HCM");
  const [seoTitle, setSeoTitle] = useState("ZPOS - Phần mềm quản lý bán hàng");
  const [seoDescription, setSeoDescription] = useState("Giải pháp quản lý bán hàng toàn diện cho cửa hàng thời trang");
  const [socialFacebook, setSocialFacebook] = useState("https://facebook.com/zpos");
    ...
    try {
      const customCss = `
        :root {
          ...
          --font-family-primary: "${themeConfig.fontFamily}", sans-serif;
          --button-radius: ${themeConfig.buttonStyle === 'rounded-none' ? '0px' : themeConfig.buttonStyle === 'rounded-md' ? '0.375rem' : '9999px'};
        }
      ...
      const res = await fetch("/api/admin/theme-config", {
        ...
        body: JSON.stringify({
          ...
          storefront_theme: themeConfig.productCardLayout
        })
      });
      const json = await res.json();
      
      if (json.success) {
        toast.success("Đã lưu cấu hình Giao diện lên Storefront thành công!");
      } else {
        toast.error("Không thể lưu cấu hình: " + json.error);
      }
    } catch (err: any) {
  ...
  // --- 0. SIZE GUIDE MANAGER ---
  const renderSizeGuideManager = () => {
    return (
      <div className="space-y-6 w-full mx-auto animate-fadeIn">
        <Card className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 shadow-[0_8px_30px_rgb(0,0,0,0.02)] rounded-xl overflow-hidden">
          <CardHeader className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800 p-6">
            <CardTitle className="text-xl font-bold flex items-center gap-2 text-slate-800 dark:text-slate-100">
              <Ruler className="w-5 h-5 text-indigo-600" /> Cấu hình Hướng dẫn chọn size
            </CardTitle>
            <CardDescription className="text-slate-500">
              Tùy chỉnh thông số hiển thị bảng chọn size và các khuyến nghị size theo chiều cao, cân nặng của khách hàng
              trên Storefront.
            </CardDescription>
          </CardHeader>
          <CardContent className="p-6 space-y-6">
            <div className="bg-indigo-50 dark:bg-indigo-500/10 p-4 rounded-lg border border-indigo-100 dark:border-indigo-500/20">
              <p className="text-sm font-medium text-indigo-800 dark:text-indigo-300 flex gap-2">
                <Sparkles className="w-5 h-5 shrink-0" />
                Tính năng này hiện tại đang sử dụng cấu hình bảng size chuẩn tự động (S, M, L, XL, 2XL) trong
                Storefront. Dữ liệu này có thể được đồng bộ hóa từ API trong các bản cập nhật sắp tới.
              </p>
            </div>

            <div className="space-y-4 border rounded-xl p-4 md:p-6 bg-slate-50/50 dark:bg-slate-900/50">
              <h3 className="text-sm font-bold text-slate-700 dark:text-slate-200">Trạng thái công cụ</h3>
              <div className="flex items-center justify-between p-4 bg-white dark:bg-slate-950 rounded-lg border shadow-sm">
                <div>
                  <p className="font-bold text-sm text-slate-800 dark:text-slate-200">
                    Kích hoạt tiện ích "Hướng dẫn chọn size"
                  </p>
                  <p className="text-xs text-slate-500 mt-1">
                    Hiển thị nút Hướng dẫn ngay trên phần chọn Kích thước ở trang Chi tiết sản phẩm.
                  </p>
                </div>
                <Switch checked={true} />
              </div>
            ...
            <div className="flex justify-end pt-4">
              <Button className="bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold px-8 shadow-md">
                <Save className="w-4 h-4 mr-2" /> Lưu thiết lập
              </Button>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-blue-700 via-indigo-700 to-indigo-950 p-6 md:p-8 text-white shadow-xl">
          ...
          <div className="relative z-10 max-w-2xl space-y-3">
            <Badge className="bg-white/20 text-white border-none backdrop-blur-md px-3 py-1 font-bold text-xs uppercase tracking-wider">
              <Sparkles className="w-3.5 h-3.5 mr-1 text-yellow-300 animate-pulse fill-yellow-300" /> Bản phát hành
              v1.4.12
            </Badge>
            <h1 className="text-3xl md:text-4xl font-black tracking-tight leading-tight">
              Chào mừng quay trở lại, Admin ZPOS!
            </h1>
            <p className="text-indigo-100 font-medium text-sm md:text-base">
              Hôm nay hệ thống ghi nhận sự tăng trưởng ổn định. Có{" "}
              <span className="font-bold text-yellow-300 underline">{newInqs} yêu cầu tư vấn mới</span> đang chờ xử lý
              từ trang giới thiệu sản phẩm.
            </p>
          </div>
        </div>
          ...
          {[
            {
              ...
              value: "24,890",
              change: "+12.4%",
              desc: "7 ngày gần nhất",
              color: "from-blue-600 to-blue-400",
              icon: Eye,
            },
            {
              title: "Yêu cầu tư vấn",
              value: `${inquiries.length} Lead`,
              change: `+${newInqs} mới`,
              desc: "Tỉ lệ phản hồi 100%",
              color: "from-emerald-600 to-emerald-400",
              icon: MessageSquare,
            },
            {
              title: "Bài viết Blog",
              value: `${blogPosts.length} Bài`,
              change: "1 Bản nháp",
              desc: "Tối ưu hóa SEO chuẩn AI",
              color: "from-indigo-600 to-indigo-400",
              icon: FileText,
            },
            {
              title: "Tài nguyên Media",
              value: `${mediaItems.length} Tệp`,
              change: "Tổng 6.2 MB",
              desc: "Đã tối ưu hóa CDN",
            ...
            <Card
              key={i}
              className="relative overflow-hidden bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 shadow-[0_8px_30px_rgb(0,0,0,0.02)] dark:shadow-[0_20px_40px_rgba(0,0,0,0.2)] rounded-xl group hover:scale-[1.02] transition-transform"
            >
              ...
              <CardContent className="pt-2">
                ...
                <div className="flex items-center gap-2 mt-1">
                  <Badge
                    variant="secondary"
                    className="bg-slate-100 dark:bg-slate-800 text-[10px] font-bold text-indigo-600 dark:text-indigo-400 px-2 py-0.5"
                  >
                    {card.change}
                  </Badge>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              <CardHeader className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between pb-2">
                <div>
                  ...
                  <CardDescription className="text-xs font-semibold text-slate-400">
                    Báo cáo đồ thị tiếp thị thời gian thực
                  </CardDescription>
                </div>
                <TabsList className="bg-slate-100 dark:bg-slate-800 p-1 rounded-lg">
                  <TabsTrigger value="traffic" className="text-xs font-bold px-3 py-1.5">
                    Truy cập
                  </TabsTrigger>
                  <TabsTrigger value="comparison" className="text-xs font-bold px-3 py-1.5">
                    So sánh tuần
                  </TabsTrigger>
                  <TabsTrigger value="hourly" className="text-xs font-bold px-3 py-1.5">
                    Giờ cao điểm
                  </TabsTrigger>
                  <TabsTrigger value="category" className="text-xs font-bold px-3 py-1.5">
                    Chuyên mục
                  </TabsTrigger>
                </TabsList>
              </CardHeader>
                      ...
                      <BarChart data={hourlyLeadData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                        ...
                        <Bar dataKey="leads" name="Yêu cầu tư vấn" fill="hsl(var(--primary))" radius={[6, 6, 0, 0]}>
                          {hourlyLeadData.map((entry, index) => (
                            <Cell
                              key={`cell-${index}`}
                              fill={index === 4 ? "hsl(var(--primary))" : "hsl(var(--primary) / 0.6)"}
                            />
                          ))}
                        </Bar>
                      </BarChart>
          ...
          <Card className="lg:col-span-4 bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 shadow-[0_8px_30px_rgb(0,0,0,0.02)] dark:shadow-[0_20px_40px_rgba(0,0,0,0.2)] rounded-xl">
            <CardHeader>
              ...
              <CardDescription className="text-xs font-semibold text-slate-400">
                Các tác vụ thường nhật của quản trị viên
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3 pt-2">
              <Button
                asChild
                className="w-full bg-slate-900 dark:bg-slate-800 hover:bg-blue-600 dark:hover:bg-blue-600 text-white rounded-lg justify-between h-11 px-4 transition-colors font-bold shadow-md"
              >
                <a href="#landing-page">
              ...
              <Button
                ...
                className="w-full bg-slate-900 dark:bg-slate-800 hover:bg-indigo-600 dark:hover:bg-indigo-600 text-white rounded-lg justify-between h-11 px-4 transition-colors font-bold shadow-md"
              >
                <span className="flex items-center gap-2">
              ...
              <Button
                asChild
                className="w-full bg-slate-900 dark:bg-slate-800 hover:bg-emerald-600 dark:hover:bg-emerald-600 text-white rounded-lg justify-between h-11 px-4 transition-colors font-bold shadow-md"
              >
                <a href="#inquiries">
                  <span className="flex items-center gap-2">
              ...
              <Button
                onClick={handleUploadFakeMedia}
                className="w-full bg-slate-900 dark:bg-slate-800 hover:bg-amber-600 dark:hover:bg-amber-600 text-white rounded-lg justify-between h-11 px-4 transition-colors font-bold shadow-md"
              >
                <span className="flex items-center gap-2">
        ...
        <div className="lg:col-span-4 bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 shadow-[0_8px_30px_rgb(0,0,0,0.02)] dark:shadow-[0_20px_40px_rgba(0,0,0,0.2)] rounded-xl p-5 md:p-6 space-y-6 overflow-y-auto max-h-[850px] scrollbar-thin">
          ...
          <div className="space-y-4">
            <h3 className="text-xs font-black uppercase tracking-wider text-indigo-600 dark:text-indigo-400">
              1. Thương hiệu & Đầu trang
            </h3>
            <div className="space-y-2">
              <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400">
                Logo hiển thị (Text Logo)
              </label>
              <Input
                value={landingConfig.logoText}
                onChange={(e) => setLandingConfig({ ...landingConfig, logoText: e.target.value })}
                className="rounded-xl h-10 font-medium text-xs dark:bg-slate-950 border-slate-200/80"
              />
            </div>
            <div className="space-y-2">
              <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400">
              ...
              <Input
                value={landingConfig.supportPhone}
                onChange={(e) => setLandingConfig({ ...landingConfig, supportPhone: e.target.value })}
                className="rounded-xl h-10 font-medium text-xs dark:bg-slate-950 border-slate-200/80"
              />
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                rows={3}
                className="w-full p-3 rounded-xl text-xs font-medium dark:bg-slate-950 border border-slate-200 dark:border-slate-800 focus:outline-none focus:ring-1 focus:ring-blue-500"
              />
            </div>
            <div className="space-y-2">
              <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400">
              ...
              <textarea
                value={landingConfig.heroSub}
                onChange={(e) => setLandingConfig({ ...landingConfig, heroSub: e.target.value })}
                rows={4}
                className="w-full p-3 rounded-xl text-xs font-medium dark:bg-slate-950 border border-slate-200 dark:border-slate-800 focus:outline-none focus:ring-1 focus:ring-blue-500"
              />
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400">
                Màu sắc chủ đạo (Accent Hex Color)
              </label>
              <div className="flex gap-2">
                <Input
                  value={landingConfig.accentColor}
                  onChange={(e) => setLandingConfig({ ...landingConfig, accentColor: e.target.value })}
                  className="rounded-xl h-10 font-bold text-xs dark:bg-slate-950 border-slate-200/80"
                />
                <input
                  type="color"
                  value={landingConfig.accentColor}
                  onChange={(e) => setLandingConfig({ ...landingConfig, accentColor: e.target.value })}
                  className="w-10 h-10 p-0 rounded-xl border border-slate-200 cursor-pointer overflow-hidden bg-transparent"
                />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setLandingConfig({ ...landingConfig, primaryButtonText: e.target.value })}
                  className="rounded-xl h-10 text-xs dark:bg-slate-950 border-slate-200/80"
                />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-slate-500 dark:text-slate-400">Nút phụ (Secondary)</label>
                <Input
                  value={landingConfig.secondaryButtonText}
                  onChange={(e) => setLandingConfig({ ...landingConfig, secondaryButtonText: e.target.value })}
                  className="rounded-xl h-10 text-xs dark:bg-slate-950 border-slate-200/80"
                />
              </div>
            </div>
            ...
            <div className="flex items-center justify-between p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
              <div>
                ...
                <span className="text-[9px] font-semibold text-slate-400 block mt-0.5">
                  Tắt để chuyển sang nút Liên hệ trực tiếp
                </span>
              </div>
              <Switch
                checked={landingConfig.showPricing}
                onCheckedChange={(val) => setLandingConfig({ ...landingConfig, showPricing: val })}
              />
            </div>
            ...
            {landingConfig.showPricing && (
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <Input
                    value={landingConfig.pricingPlanBasic}
                    onChange={(e) => setLandingConfig({ ...landingConfig, pricingPlanBasic: e.target.value })}
                    className="rounded-xl h-9 text-xs dark:bg-slate-950 border-slate-200/80"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-500 dark:text-slate-400">Gói Chuyên nghiệp</label>
                  <Input
                    value={landingConfig.pricingPlanPro}
                    onChange={(e) => setLandingConfig({ ...landingConfig, pricingPlanPro: e.target.value })}
                    className="rounded-xl h-9 text-xs dark:bg-slate-950 border-slate-200/80"
                  />
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onChange={(e) => setLandingConfig({ ...landingConfig, promoCode: e.target.value })}
                  className="rounded-xl h-9 text-xs dark:bg-slate-950 border-slate-200/80"
                />
              </div>
              <div className="space-y-1">
                ...
                <Input
                  value={landingConfig.promoDiscount}
                  onChange={(e) => setLandingConfig({ ...landingConfig, promoDiscount: e.target.value })}
                  className="rounded-xl h-9 text-xs dark:bg-slate-950 border-slate-200/80"
                />
              </div>
            </div>
          ...
          <div className="pt-4 border-t dark:border-slate-800 flex gap-3">
            <Button
              onClick={() => handleSaveLandingConfig(false)}
              variant="outline"
              className="flex-1 rounded-lg h-11 text-xs font-bold border-slate-200"
            >
              <Save className="w-4 h-4 mr-2" /> Lưu nháp
            </Button>
            <Button
              onClick={() => handleSaveLandingConfig(true)}
              className="flex-1 rounded-lg h-11 text-xs font-bold bg-blue-600 hover:bg-blue-700 text-white shadow-md"
            >
        ...
        <div className="lg:col-span-8 flex flex-col space-y-4 h-full max-h-[850px]">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 shadow-[0_8px_30px_rgb(0,0,0,0.02)] dark:shadow-[0_20px_40px_rgba(0,0,0,0.2)] rounded-xl p-4 flex justify-between items-center shrink-0">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs font-bold text-slate-600 dark:text-slate-400">
                Khung xem trước màn hình (Live Site Preview)
              </span>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                onClick={() => setLandingConfig({ ...landingConfig, isDarkPreview: !landingConfig.isDarkPreview })}
                variant="outline"
                size="sm"
                className="rounded-lg text-[11px] font-bold border-slate-200"
              >
          ...
          <div className="flex-1 bg-slate-200 dark:bg-slate-950 rounded-xl overflow-hidden p-6 border border-slate-300/40 dark:border-slate-800/80 flex items-center justify-center max-h-[720px] shadow-inner">
            ...
            >
              ...
              <div
                className={`flex-1 overflow-y-auto select-none transition-colors duration-300 ${landingConfig.isDarkPreview ? "bg-slate-950 text-white" : "bg-slate-50 text-slate-900"}`}
              >
                {/* Navbar mockup */}
                <div className="px-6 py-4 flex items-center justify-between border-b border-slate-100/10 shrink-0">
                  <span className="text-sm font-black tracking-tight" style={{ color: landingConfig.accentColor }}>
                    {landingConfig.logoText}
                  </span>
                  ...
                  <Button
                    size="sm"
                    className="rounded-full text-[9px] font-black h-7 px-3 text-white"
                    style={{ backgroundColor: landingConfig.accentColor }}
                  >
                    Đăng ký
                  </Button>
                </div>
                ...
                <div className="px-8 py-16 text-center space-y-6 max-w-lg mx-auto">
                  <Badge
                    variant="outline"
                    className="text-[9px] font-bold border-indigo-500/20 text-indigo-400 bg-indigo-500/5 px-2 py-0.5"
                  >
                  ...
                  <div className="flex justify-center gap-3 pt-2">
                    <Button
                      size="sm"
                      className="rounded-full text-[10px] font-bold h-9 px-4 text-white"
                      style={{ backgroundColor: landingConfig.accentColor }}
                    >
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      className="rounded-full text-[10px] font-bold h-9 px-4 border-slate-800 bg-transparent text-slate-400"
                    >
                ...
                <div className="mx-8 my-6 p-5 rounded-lg bg-indigo-600/10 border border-indigo-500/20 text-center space-y-2">
                  ...
                  <div className="text-xs text-slate-300 font-medium">
                    {landingConfig.promoDiscount} khi thanh toán năm
                  </div>
                </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="p-4 rounded-xl bg-slate-900 border border-slate-800 space-y-2">
                        ...
                        <div className="text-[8px] text-slate-500 leading-tight">
                          Phù hợp hộ kinh doanh nhỏ lẻ, 1 chi nhánh cửa hàng độc lập.
                        </div>
                      </div>
                      <div
                        className="p-4 rounded-xl bg-slate-900 border-2 space-y-2"
                        style={{ borderColor: landingConfig.accentColor }}
                      >
                        <div className="text-[9px] font-bold" style={{ color: landingConfig.accentColor }}>
                          ZPOS PRO
                        </div>
                        <div className="text-sm font-black text-slate-100">{landingConfig.pricingPlanPro}</div>
                        <div className="text-[8px] text-slate-500 leading-tight">
                          Cho chuỗi cửa hàng lớn, quản lý kho tự động đa địa điểm & Voice AI.
                        </div>
                      </div>
                    </div>
                  </div>
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between border-b pb-4 dark:border-slate-800">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-muted-foreground text-sm">
              Quản lý các tin tức và chiến dịch bài viết tiếp thị PR của thương hiệu ZPOS.
            </p>
          </div>
          <Button
            onClick={handleCreateNewBlog}
            className="rounded-xl h-10 px-4 text-xs font-bold bg-blue-600 hover:bg-blue-700 text-white shadow-sm"
          >
                ...
                filteredBlogs.map((post) => (
                  <TableRow key={post.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/40">
                    <TableCell>
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col min-w-0">
                          <span className="font-bold text-slate-800 dark:text-slate-100 truncate max-w-[320px]">
                            {post.title}
                          </span>
                    ...
                    <TableCell>
                      <Badge
                        variant="outline"
                        className="text-[10px] font-bold px-1.5 border-slate-200 dark:border-slate-800"
                      >
                        {post.category}
                      </Badge>
                    </TableCell>
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          >
                            ...
                            {post.status === "published" ? "Chuyển thành bản nháp" : "Yêu cầu đăng tải"}
                          </DropdownMenuItem>
                          <DropdownMenuSeparator />
                          <DropdownMenuItem
        ...
        <AlertDialog open={deleteBlogAlertOpen} onOpenChange={setDeleteBlogAlertOpen}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Hành động này sẽ gỡ bỏ hoàn toàn bài viết này ra khỏi website giới thiệu ZPOS. Bạn không thể khôi phục
                lại dữ liệu này.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
      ...
      <div className="space-y-6 animate-fadeIn pb-12">
        <div className="flex justify-between items-center border-b pb-4 dark:border-slate-800">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-muted-foreground text-sm">
              Soạn thảo trực quan, tự động kiểm tra tối ưu hóa SEO và độ tương phản từ khóa.
            </p>
          </div>
          <div className="flex gap-2">
            <Button
            ...
            <Button
              ...
              className="rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold px-4 shadow-sm"
            >
              Lưu bài viết
            </Button>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-8 bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 shadow-[0_8px_30px_rgb(0,0,0,0.02)] dark:shadow-[0_20px_40px_rgba(0,0,0,0.2)] rounded-xl p-5 md:p-6 space-y-6">
            <div className="space-y-2">
              <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400">
                Tiêu đề bài viết (Blog Title)
              </label>
              <Input
                value={editorTitle}
                onChange={(e) => setEditorTitle(e.target.value)}
                placeholder="Nhập tiêu đề hấp dẫn..."
                className="rounded-xl h-11 text-xs font-bold dark:bg-slate-950 border-slate-200/80"
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                ...
                <select
                  ...
                  className="w-full h-10 px-3 rounded-xl text-xs font-bold dark:bg-slate-950 border border-slate-200 dark:border-slate-800 bg-transparent focus:outline-none"
                >
                  <option value="Kinh doanh">Kinh doanh</option>
                  <option value="Công nghệ">Công nghệ</option>
                  <option value="Quản lý">Quản lý</option>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="tieu-de-viet-lien-khong-dau"
                  className="rounded-xl h-10 text-xs font-semibold dark:bg-slate-950 border-slate-200/80"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="5 phút"
                  className="rounded-xl h-10 text-xs font-medium dark:bg-slate-950 border-slate-200/80"
                />
              </div>
              <div className="space-y-2">
                <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400">
                  Tệp ảnh đại diện (Cover Image URL)
                </label>
                <Input
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                placeholder="Nhập một đoạn tóm tắt ngắn làm mô tả SEO bài viết..."
                rows={2}
                className="w-full p-3 rounded-xl text-xs font-medium dark:bg-slate-950 border border-slate-200 dark:border-slate-800 focus:outline-none focus:ring-1 focus:ring-blue-500"
              />
            </div>
            ...
            <div className="space-y-2">
              <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400">
                Nội dung chi tiết bài viết (Markdown supported)
              </label>
              <textarea
                value={editorContent}
                ...
                rows={12}
                className="w-full p-4 rounded-xl text-xs font-medium dark:bg-slate-950 border border-slate-200 dark:border-slate-800 focus:outline-none focus:ring-1 focus:ring-indigo-500 font-mono"
              />
            </div>
          </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <Card className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 shadow-[0_8px_30px_rgb(0,0,0,0.02)] dark:shadow-[0_20px_40px_rgba(0,0,0,0.2)] rounded-xl overflow-hidden">
              <CardHeader className="bg-gradient-to-r from-violet-600 to-indigo-600 text-white">
                ...
                <CardDescription className="text-indigo-100 text-[10px] font-bold uppercase tracking-wider">
                  Phân tích điểm SEO tự động
                </CardDescription>
              </CardHeader>
              <CardContent className="p-6 space-y-6">
                  ...
                  <div className="relative w-28 h-28 flex items-center justify-center">
                    <svg className="w-full h-full transform -rotate-90">
                      ...
                      <circle
                        ...
                        strokeDashoffset={301.6 - (301.6 * seoAnalysis.score) / 100}
                        strokeLinecap="round"
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {seoAnalysis.keywords.map((kw, i) => (
                      <Badge
                        key={i}
                        variant="secondary"
                        className="text-[9px] bg-slate-100 dark:bg-slate-800 border-none font-bold text-slate-600 dark:text-slate-400 px-2 py-0.5"
                      >
                        {kw}
                      </Badge>
                    ))}
                ...
                <div className="space-y-2 pt-2">
                  <Button
                    onClick={triggerAiSeoAnalyze}
                    variant="outline"
                    className="w-full rounded-xl h-10 text-xs font-bold border-slate-200"
                  >
                  ...
                  <Button
                    ...
                    variant="outline"
                    className="w-full rounded-xl h-10 text-xs font-bold border-slate-200 text-indigo-600 hover:text-indigo-700"
                  >
            ...
            <div className="flex gap-2">
              <div className="relative w-44">
                ...
                <Input
                  ...
                  className="rounded-xl pl-9 h-9 text-xs font-medium dark:bg-slate-950 border-slate-200/80"
                />
              </div>
              <Button
                onClick={handleUploadFakeMedia}
                className="rounded-xl h-9 text-xs font-bold bg-blue-600 text-white"
              >
              ...
              .map((item) => {
                ...
                return (
                  <div
                    ...
                    className={`group rounded-lg overflow-hidden border p-2 flex flex-col cursor-pointer transition-all ${
                      isSelected
                        ? "border-blue-650 bg-blue-50/50 dark:bg-blue-950/20"
                        : "border-slate-200/60 dark:border-slate-800/80 hover:border-slate-400 hover:scale-[1.02]"
                    }`}
                  >
                    ...
                    <div className="pt-2 flex-1 flex flex-col justify-between">
                      <span className="text-[10px] font-black text-slate-800 dark:text-slate-100 truncate block">
                        {item.name}
                      </span>
        ...
        <div className="lg:col-span-4 h-full max-h-[800px]">
          <Card className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 shadow-[0_8px_30px_rgb(0,0,0,0.02)] dark:shadow-[0_20px_40px_rgba(0,0,0,0.2)] rounded-xl h-full flex flex-col overflow-hidden">
            <CardHeader className="border-b dark:border-slate-800 shrink-0">
              ...
              <CardDescription className="text-xs font-semibold text-slate-400">
                Xem và sao chép mã liên kết CDN
              </CardDescription>
            </CardHeader>
              ...
              <CardContent className="p-5 flex-1 flex flex-col justify-between overflow-y-auto">
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    {[
                      ...
                      <div
                        key={i}
                        className="flex justify-between items-center text-xs border-b pb-2 dark:border-slate-800 last:border-0"
                      >
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-wider text-slate-400">
                      Đường dẫn CDN công khai
                    </label>
                    <div className="flex gap-1.5 bg-slate-50 dark:bg-slate-950 p-2 rounded-xl border border-slate-200/60 dark:border-slate-850">
                      <span className="text-[10px] font-bold text-slate-500 truncate flex-1 leading-6">
                        {selectedMedia.url}
                      </span>
                      <Button
                        onClick={() => handleCopyText(selectedMedia.url, "URL Media")}
                        size="icon"
                        variant="ghost"
                ...
                <div className="pt-6 border-t dark:border-slate-800 shrink-0">
                  <Button
                    ...
                    className="w-full bg-red-500 hover:bg-red-600 text-white rounded-lg h-11 text-xs font-bold"
                  >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-full max-h-[850px] animate-fadeIn pb-6">
        ...
        <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 shadow-[0_8px_30px_rgb(0,0,0,0.02)] dark:shadow-[0_20px_40px_rgba(0,0,0,0.2)] rounded-xl p-5 md:p-6 space-y-6 flex flex-col h-full max-h-[800px]">
          <div className="flex flex-col gap-1 border-b pb-4 dark:border-slate-800 shrink-0">
            ...
            <p className="text-muted-foreground text-sm">
              Danh sách đăng ký tư vấn khách hàng tiềm năng nhận được từ trang giới thiệu sản phẩm ZPOS.
            </p>
          </div>
                      ...
                      >
                        ...
                        <TableCell>
                          <Badge
                            ...
                            className={
                              inq.status === "new"
                                ? "text-[10px] px-1.5 h-5 bg-red-500/10 text-red-600 border-none font-bold uppercase tracking-wider"
                                : inq.status === "contacted"
                                  ? "text-[10px] px-1.5 h-5 bg-amber-500/10 text-amber-600 border-none font-bold uppercase tracking-wider"
        ...
        <div className="lg:col-span-5 h-full max-h-[800px]">
          <Card className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 shadow-[0_8px_30px_rgb(0,0,0,0.02)] dark:shadow-[0_20px_40px_rgba(0,0,0,0.2)] rounded-xl h-full flex flex-col overflow-hidden">
            <CardHeader className="border-b dark:border-slate-800 shrink-0">
              ...
              <CardDescription className="text-xs font-semibold text-slate-400">
                Thông tin liên lạc & dòng lịch sử cuộc gọi
              </CardDescription>
            </CardHeader>
                  ...
                  <div className="p-4 rounded-lg bg-slate-50 dark:bg-slate-950 border space-y-3">
                    ...
                    <div className="flex items-center justify-between text-xs border-t pt-2.5 dark:border-slate-850">
                      <span className="flex items-center gap-2.5">
                        ...
                        <span className="font-semibold text-slate-600 dark:text-slate-400 truncate max-w-[180px]">
                          {selectedInquiry.customerEmail}
                        </span>
                      </span>
                      <Button
                        onClick={() => handleCopyText(selectedInquiry.customerEmail, "Email")}
                        size="icon"
                        variant="ghost"
                        className="w-6 h-6 rounded-lg"
                      >
                    ...
                    <div className="flex items-center justify-between text-xs border-t pt-2.5 dark:border-slate-850">
                      ...
                      <Button
                        onClick={() => handleCopyText(selectedInquiry.customerPhone, "Số điện thoại")}
                        size="icon"
                        variant="ghost"
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2 max-h-[160px] overflow-y-auto pr-1">
                      {selectedInquiry.notes.map((note, idx) => (
                        <div
                          key={idx}
                          className="p-3 rounded-lg bg-indigo-50/60 dark:bg-slate-950 text-xs border-l-4 border-indigo-500 font-semibold text-slate-600 dark:text-slate-400"
                        >
                          {note}
                        </div>
                      ))}
                ...
                <div className="pt-6 border-t dark:border-slate-800 space-y-2 shrink-0">
                  <div className="flex gap-2">
                    <Input
                      ...
                      className="rounded-xl h-10 text-xs font-medium dark:bg-slate-950 border-slate-200/80"
                      onKeyDown={(e) => e.key === "Enter" && handleAddInquiryNote()}
                    />
                    <Button
                      onClick={handleAddInquiryNote}
                      size="icon"
    ...
    return (
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-full max-h-[850px] animate-fadeIn pb-6">
        ...
        <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 shadow-[0_8px_30px_rgb(0,0,0,0.02)] dark:shadow-[0_20px_40px_rgba(0,0,0,0.2)] rounded-xl p-5 md:p-6 space-y-6 overflow-y-auto max-h-[800px] scrollbar-thin">
          <div className="border-b pb-4 dark:border-slate-800 flex justify-between items-start">
            <div>
              <h2 className="text-lg font-black text-slate-800 dark:text-slate-100">
                Cài đặt chung (General Settings)
              </h2>
              <p className="text-[11px] font-semibold text-slate-400 mt-1">
                Quản lý thông tin cửa hàng, SEO, tên miền và kết nối ngoại vi
              </p>
            </div>
            <Button
              onClick={() => toast.success("Đã lưu cài đặt hệ thống!")}
              className="rounded-xl h-9 text-xs font-bold bg-blue-600 text-white"
            >
          ...
          <div className="space-y-4">
            <h3 className="text-xs font-black uppercase tracking-wider text-indigo-600 dark:text-indigo-400 flex items-center gap-2">
              <Store className="w-4 h-4" /> 1. Thông tin cửa hàng (Store Info)
            </h3>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400">Tên cửa hàng</label>
                <Input
                  value={storeName}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400">Email liên hệ</label>
                <Input
                  value={contactEmail}
              ...
              <div className="space-y-2">
                <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400">Facebook Page URL</label>
                <Input
            ...
            <div className="space-y-2">
              <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400">Địa chỉ cửa hàng</label>
              <Input
                value={storeAddress}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400">
                  Đường dẫn Favicon (Favicon URL)
                </label>
                <Input
                  value={storeFavicon}
                  onChange={(e) => setStoreFavicon(e.target.value)}
                  className="rounded-xl h-10 font-bold text-xs dark:bg-slate-950 border-slate-200/80"
                />
              </div>
            </div>
          ...
          <div className="space-y-4 pt-4 border-t dark:border-slate-800">
            <h3 className="text-xs font-black uppercase tracking-wider text-indigo-600 dark:text-indigo-400 flex items-center gap-2">
              <Search className="w-4 h-4" /> 2. Cấu hình SEO mặc định (Default SEO)
            </h3>
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                rows={2}
                className="w-full p-3 rounded-xl text-xs font-bold dark:bg-slate-950 border border-slate-200 dark:border-slate-800 focus:outline-none focus:ring-1 focus:ring-indigo-500"
              />
            </div>
          </div>
          ...
          <div className="space-y-4 pt-4 border-t dark:border-slate-800">
            <h3 className="text-xs font-black uppercase tracking-wider text-indigo-600 dark:text-indigo-400 flex items-center gap-2">
              <Globe className="w-4 h-4" /> 3. Tên miền trỏ về (Custom Domain Mapping)
            </h3>
          ...
          <div className="space-y-4 pt-4 border-t dark:border-slate-800">
            <h3 className="text-xs font-black uppercase tracking-wider text-indigo-600 dark:text-indigo-400 flex items-center gap-2">
              <Mail className="w-4 h-4" /> 4. Cổng thư báo giá tự động (SMTP Server)
            </h3>
        ...
        <div className="lg:col-span-5 h-full max-h-[800px]">
          <Card className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 shadow-[0_8px_30px_rgb(0,0,0,0.02)] dark:shadow-[0_20px_40px_rgba(0,0,0,0.2)] rounded-xl h-full flex flex-col overflow-hidden">
            <CardHeader className="border-b dark:border-slate-800 shrink-0">
              <CardTitle className="text-base font-bold text-slate-800 dark:text-slate-100">
                Bản sao lưu cơ sở dữ liệu (Backups)
              </CardTitle>
              <CardDescription className="text-xs font-semibold text-slate-400">
                Quản lý các điểm phục hồi vật lý
              </CardDescription>
            </CardHeader>
            ...
            <CardContent className="p-5 flex-1 flex flex-col justify-between overflow-y-auto">
              <div className="space-y-4">
                <div className="flex justify-between items-center shrink-0">
                  <span className="text-[10px] font-black uppercase tracking-wider text-slate-400">
                    Lịch sử sao lưu gần nhất
                  </span>
                  <Button
                    onClick={handleTriggerBackup}
                    size="sm"
                    className="rounded-xl text-[10px] font-bold h-8 bg-blue-600 text-white"
                  >
                ...
                <div className="space-y-2 overflow-y-auto max-h-[420px] pr-1">
                  {backups.map((bk) => (
                    <div
                      key={bk.id}
                      className="p-3 rounded-lg bg-slate-50 dark:bg-slate-950 border flex justify-between items-center text-xs"
                    >
                      <div>
                        <span className="font-bold text-slate-800 dark:text-slate-200 block truncate max-w-[160px]">
                          {bk.version}
                        </span>
              ...
              <div className="p-4 rounded-lg bg-blue-50/50 dark:bg-slate-950 border border-blue-200/20 text-[11px] font-semibold leading-relaxed text-slate-600 dark:text-slate-400 shrink-0">
                ...
                Mỗi tệp tin sao lưu được mã hóa theo chuẩn AES-256 và lưu trữ cách ly hoàn toàn. Vui lòng liên hệ quản
                trị hệ thống để kích hoạt điểm khôi phục (Restore Point).
              </div>
            </CardContent>
          </Card>
        </div>
  ...
  // --- 6. UI DESIGN MANAGER ---
  const renderUIDesign = () => {
    return (
      <div className="space-y-6 w-full mx-auto animate-fadeIn pb-10">
        <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-slate-900 via-slate-800 to-indigo-950 p-6 md:p-8 text-white shadow-xl">
          <div className="absolute top-0 right-0 -mr-20 -mt-20 w-80 h-80 bg-blue-500/20 rounded-full blur-3xl" />
          <div className="relative z-10 max-w-2xl space-y-3">
            <h1 className="text-2xl md:text-3xl font-black tracking-tight leading-tight flex items-center gap-3">
              <Palette className="w-8 h-8 text-blue-400" /> Thiết kế Giao diện (UI Design)
            </h1>
            <p className="text-slate-300 font-medium text-sm md:text-base">
              Tùy chỉnh toàn diện thiết kế trực quan của Cửa hàng trực tuyến (Storefront) bao gồm màu sắc, phông chữ và bố cục.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <div className="lg:col-span-4 space-y-6">
            <Card className="bg-white dark:bg-slate-900 shadow-[0_8px_30px_rgb(0,0,0,0.02)] border-slate-200/60 rounded-xl overflow-hidden">
              <CardHeader className="bg-slate-50 dark:bg-slate-800/50 border-b p-5">
                <CardTitle className="text-base font-bold flex items-center gap-2">
                  <Palette className="w-4 h-4 text-blue-600" /> Bộ màu sắc (Colors)
                </CardTitle>
              </CardHeader>
              <CardContent className="p-5 space-y-4">
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-500">Màu chủ đạo (Primary Color)</label>
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-500">Màu phụ trợ (Secondary Color)</label>
                  <div className="flex gap-2 items-center">
                    <input 
                      type="color" 
            ...
            <Card className="bg-white dark:bg-slate-900 shadow-[0_8px_30px_rgb(0,0,0,0.02)] border-slate-200/60 rounded-xl overflow-hidden">
              <CardHeader className="bg-slate-50 dark:bg-slate-800/50 border-b p-5">
                <CardTitle className="text-base font-bold flex items-center gap-2">
                  <Type className="w-4 h-4 text-indigo-600" /> Typography & Kiểu nút
                </CardTitle>
              </CardHeader>
              <CardContent className="p-5 space-y-4">
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-500">Phông chữ (Font Family)</label>
                  <select 
                  ...
                  >
                    ...
                    <option value="Playfair Display">Playfair Display (Thời trang)</option>
                  </select>
                </div>
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-500">Bo góc nút bấm (Button Style)</label>
                  <div className="flex gap-2 mt-2">
                    <button 
                      onClick={() => setThemeConfig({ ...themeConfig, buttonStyle: "rounded-none" })}
                      className={`flex-1 py-2 px-3 text-xs font-semibold border ${themeConfig.buttonStyle === 'rounded-none' ? 'border-indigo-600 bg-indigo-50 text-indigo-700' : 'border-slate-200'}`}
                    >
                      Vuông
                    </button>
                    <button 
                      onClick={() => setThemeConfig({ ...themeConfig, buttonStyle: "rounded-md" })}
                      className={`flex-1 py-2 px-3 text-xs font-semibold border rounded-md ${themeConfig.buttonStyle === 'rounded-md' ? 'border-indigo-600 bg-indigo-50 text-indigo-700' : 'border-slate-200'}`}
                    >
                      Bo nhẹ
                    </button>
                    <button 
                      onClick={() => setThemeConfig({ ...themeConfig, buttonStyle: "rounded-full" })}
                      className={`flex-1 py-2 px-3 text-xs font-semibold border rounded-full ${themeConfig.buttonStyle === 'rounded-full' ? 'border-indigo-600 bg-indigo-50 text-indigo-700' : 'border-slate-200'}`}
                    >
                      Viên thuốc
                    </button>
                  </div>
          ...
          <div className="lg:col-span-8 space-y-6">
            <Card className="bg-white dark:bg-slate-900 shadow-[0_8px_30px_rgb(0,0,0,0.02)] border-slate-200/60 rounded-xl overflow-hidden h-full flex flex-col">
              <CardHeader className="bg-slate-50 dark:bg-slate-800/50 border-b p-5 flex flex-row items-center justify-between">
                <CardTitle className="text-base font-bold flex items-center gap-2">
                  <LayoutTemplate className="w-4 h-4 text-emerald-600" /> Bố cục chung (Layout Structure)
                </CardTitle>
                <Button size="sm" className="bg-indigo-600 text-white hover:bg-indigo-700 font-bold px-6 shadow-md" onClick={handleSaveThemeConfig}>
                  <Save className="w-4 h-4 mr-2" /> Lưu Giao Diện
                </Button>
              </CardHeader>
              <CardContent className="p-6 flex-1 space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-3">
                    <label className="text-sm font-bold text-slate-800 dark:text-slate-200">Kiểu Header</label>
                    <div className="grid grid-cols-2 gap-3">
                      <div 
                        className={`border rounded-lg p-3 cursor-pointer transition-all ${themeConfig.headerLayout === 'left' ? 'ring-2 ring-indigo-600 border-indigo-600 bg-indigo-50/50' : 'hover:border-slate-300'}`}
                        onClick={() => setThemeConfig({ ...themeConfig, headerLayout: 'left' })}
                      >
                        <div className="w-full h-8 bg-slate-200 dark:bg-slate-700 rounded mb-2 flex items-center px-2">
                          <div className="w-4 h-4 bg-slate-400 rounded-full mr-2"></div>
                          <div className="w-8 h-1 bg-slate-400 rounded mr-1"></div>
                          <div className="w-8 h-1 bg-slate-400 rounded mr-1"></div>
                          <div className="w-8 h-1 bg-slate-400 rounded"></div>
                        </div>
                        <p className="text-[10px] font-bold text-center text-slate-500">Logo Trái</p>
                      </div>
                      <div 
                        className={`border rounded-lg p-3 cursor-pointer transition-all ${themeConfig.headerLayout === 'centered' ? 'ring-2 ring-indigo-600 border-indigo-600 bg-indigo-50/50' : 'hover:border-slate-300'}`}
                        onClick={() => setThemeConfig({ ...themeConfig, headerLayout: 'centered' })}
                      >
                        <div className="w-full h-8 bg-slate-200 dark:bg-slate-700 rounded mb-2 flex items-center justify-between px-2">
                          <div className="flex">
                            <div className="w-6 h-1 bg-slate-400 rounded mr-1"></div>
                        ...
                        <p className="text-[10px] font-bold text-center text-slate-500">Logo Giữa</p>
                      </div>
                    </div>
                  </div>

                  <div className="space-y-3">
                    <label className="text-sm font-bold text-slate-800 dark:text-slate-200">Bố cục Card Sản phẩm</label>
                    <div className="grid grid-cols-2 gap-3">
                      <div 
                        className={`border rounded-lg p-3 cursor-pointer transition-all ${themeConfig.productCardLayout === 'classic' ? 'ring-2 ring-indigo-600 border-indigo-600 bg-indigo-50/50' : 'hover:border-slate-300'}`}
                        onClick={() => setThemeConfig({ ...themeConfig, productCardLayout: 'classic' })}
                      >
                        <div className="w-full aspect-[3/4] bg-slate-200 dark:bg-slate-700 rounded-sm mb-2"></div>
                        <div className="w-full h-2 bg-slate-300 rounded mb-1"></div>
                        <div className="w-1/2 h-2 bg-slate-300 rounded mb-2"></div>
                        <div className="w-full h-6 bg-slate-300 rounded-sm"></div>
                        <p className="text-[10px] font-bold text-center text-slate-500 mt-2">Cổ điển (Classic)</p>
                      </div>
                      <div 
                        className={`border rounded-lg p-3 cursor-pointer transition-all ${themeConfig.productCardLayout === 'minimalist' ? 'ring-2 ring-indigo-600 border-indigo-600 bg-indigo-50/50' : 'hover:border-slate-300'}`}
                        onClick={() => setThemeConfig({ ...themeConfig, productCardLayout: 'minimalist' })}
                      >
                        <div className="w-full aspect-[3/4] bg-slate-200 dark:bg-slate-700 rounded-sm mb-2"></div>
                        ...
                        <p className="text-[10px] font-bold text-center text-slate-500 mt-3">Tối giản (Minimalist)</p>
                      </div>
                    </div>
                  </div>
                </div>

                <div className="p-4 bg-slate-50 dark:bg-slate-950 border rounded-lg flex items-start gap-4">
                  <Monitor className="w-6 h-6 text-slate-400 mt-1" />
                  <div>
                    <h4 className="text-sm font-bold text-slate-700 dark:text-slate-300">Xem trước trực tiếp (Live Preview)</h4>
                    <p className="text-xs text-slate-500 mt-1 mb-3">Các thay đổi sẽ được phản ánh ngay lập tức trên trang Storefront. Bạn có thể mở tab mới để kiểm tra cấu hình trước khi lưu.</p>
                    <Button variant="outline" size="sm" className="text-xs font-semibold h-8" asChild>
                      <Link href="/products" target="_blank">
                        <ExternalLink className="w-3 h-3 mr-2" /> Mở Storefront
                      </Link>
                    </Button>
                  </div>
                </div>
  ...
  return (
    <div className="w-full h-full max-w-7xl mx-auto space-y-6">
      ...
      <div className="w-full">
        ...
        {activeTab === "ui-design" && renderUIDesign()}
        {activeTab === "sizeguide" && renderSizeGuideManager()}
