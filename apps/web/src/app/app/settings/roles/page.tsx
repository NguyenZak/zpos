...
export default function RolesPage() {
  ...
  const toggleGroupPerms = (groupPerms: Permission[]) => {
    ...
    setSelectedPerms((prev) =>
      allChecked ? prev.filter((id) => !groupIds.includes(id)) : Array.from(new Set([...prev, ...groupIds])),
    );
  };
  ...
  const getActionBadge = (action: string) => {
    switch (action) {
      case "role.create":
        return (
          <Badge className="bg-green-500/10 text-green-600 hover:bg-green-500/20 border-none font-bold text-[10px]">
            Tạo vai trò
          </Badge>
        );
      case "role.update":
        return (
          <Badge className="bg-blue-500/10 text-blue-600 hover:bg-blue-500/20 border-none font-bold text-[10px]">
            Cập nhật
          </Badge>
        );
      case "role.delete":
        return (
          <Badge className="bg-red-500/10 text-red-600 hover:bg-red-500/20 border-none font-bold text-[10px]">
            Xóa vai trò
          </Badge>
        );
      case "member.role_assign":
        return (
          <Badge className="bg-purple-500/10 text-purple-600 hover:bg-purple-500/20 border-none font-bold text-[10px]">
            Gán vai trò
          </Badge>
        );
      default:
        return (
          <Badge className="bg-gray-500/10 text-gray-600 hover:bg-gray-500/20 border-none font-bold text-[10px]">
            {action}
          </Badge>
        );
    }
      ...
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 animate-in fade-in duration-300">
        ...
        <div className="flex flex-col items-center gap-1 text-center max-w-md">
          ...
          <p className="text-muted-foreground text-sm">
            Trang Quản lý vai trò & Phân quyền yêu cầu quyền roles.manage. Vui lòng liên hệ chủ doanh nghiệp nếu bạn cần
    ...
    <div className="flex flex-col gap-6 animate-in fade-in duration-300">
                    ...
                    <div className="flex flex-col gap-3 border-t pt-4">
                      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                        <div className="flex flex-col">
                          ...
                          <p className="text-[11px] text-muted-foreground">
                            Đã chọn <span className="font-bold text-primary">{selectedPerms.length}</span> /{" "}
                      ...
                      {permLoading ? (
                        ...
                        <div className="grid gap-3 md:grid-cols-2">
                          ...
                          {Object.entries(groupedPerms).every(
                            ([, perms]) => perms.filter(matchesSearch).length === 0,
                          ) && (
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
        <TabsList className="bg-muted/60 p-1 rounded-xl mb-4 border border-muted">
          ...
          <TabsTrigger
            value="audit"
            className="rounded-lg px-4 py-1.5 font-semibold text-xs transition-all flex gap-1.5"
          >
                      ...
                      <TableRow key={role.id} className="hover:bg-muted/30 transition-colors">
                        ...
                        <TableCell className="text-center align-middle">
                          <Badge
                            ...
                            className={
                              role.is_system
                                ? "bg-primary/5 text-primary hover:bg-primary/10 border-none font-bold text-[10px]"
                                : "font-bold text-[10px] border-muted"
                            }
                          >
                ...
                {logsLoading ? (
                  ...
                  auditLogs.map((log) => (
                    <TableRow key={log.id} className="hover:bg-muted/30 transition-colors text-sm">
                      ...
                      <TableCell className="align-middle">{getActionBadge(log.action)}</TableCell>
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Hành động này không thể hoàn tác. Mọi nhân sự được gán vai trò này sẽ tạm thời bị mất quyền hạn gán và
              chuyển về quyền nhân sự mặc định.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              className="bg-destructive hover:bg-destructive/90 text-white border-none shadow-xs"
            >
              Xác nhận xóa
            </AlertDialogAction>
          </AlertDialogFooter>
