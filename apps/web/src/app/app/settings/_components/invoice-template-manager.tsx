    ...
    <div className="space-y-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="flex items-center justify-between p-3 border rounded-xl bg-muted/20">
              <div className="space-y-0.5">
                ...
                <p className="text-[10px] text-muted-foreground italic">
                  In hóa đơn ngay khi hoàn tất thanh toán.
                </p>
              </div>
            ...
            <div className="flex items-center justify-between p-3 border rounded-xl bg-muted/20">
              <div className="space-y-0.5">
                ...
                <p className="text-[10px] text-muted-foreground italic">
                  Hiển thị tùy chọn in hóa đơn cuối.
                </p>
              </div>
            ...
            <div className="grid gap-2">
              <Label htmlFor="paper-size" className="font-semibold text-sm">Khổ giấy in</Label>
              <select
                id="paper-size"
                value={paperSize}
            ...
            <div className="grid gap-2">
              <Label htmlFor="print-copies" className="font-semibold text-sm">Số liên in</Label>
              <Input 
                id="print-copies"
                type="number" 
                min="1"
      ...
      <Tabs defaultValue="payment" className="w-full">
        <TabsList className="w-full justify-start rounded-xl h-12 bg-muted/50 p-1 mb-6">
          <TabsTrigger value="payment" className="flex-1 rounded-lg font-bold data-[state=active]:bg-background data-[state=active]:text-primary data-[state=active]:shadow-sm">
            Mẫu: Hóa Đơn Thanh Toán
          </TabsTrigger>
          <TabsTrigger value="provisional" className="flex-1 rounded-lg font-bold data-[state=active]:bg-background data-[state=active]:text-primary data-[state=active]:shadow-sm">
...
function TemplateEditor({ prefix, defaultHeader, paperSize }: { prefix: string, defaultHeader: string, paperSize: string }) {
  const [saving, setSaving] = useState(false);
    ...
    <div className="grid gap-6 lg:grid-cols-12 items-start">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid gap-3 pt-2">
              <div className="grid gap-1">
                <Label htmlFor={`${prefix}-title`} className="text-xs font-bold">Nội dung Tiêu đề</Label>
                <Input
                  id={`${prefix}-title`}
                  value={headerText}
              ...
              <div className="grid gap-1">
                <Label htmlFor={`${prefix}-phone`} className="text-xs font-bold">Số điện thoại liên hệ trên hóa đơn</Label>
                <Input
                  id={`${prefix}-phone`}
                  value={bizPhone}
              ...
              <div className="grid gap-1">
                <Label htmlFor={`${prefix}-footer`} className="text-xs font-bold">Lời chào chân trang (Footer)</Label>
                <Textarea
                  id={`${prefix}-footer`}
                  value={footerText}
      ...
      <div className="lg:col-span-5 space-y-4">
        ...
        <div className="w-full flex items-center justify-center p-4 bg-muted/40 dark:bg-muted/10 rounded-2xl border-2 border-dashed border-muted-foreground/15 min-h-[500px]">
          <div className={`${getPreviewContainerClass()} ${selectedFontClass} overflow-hidden shadow-2xl relative border-black/15`}>
            ...
            <div className="text-center space-y-1 mb-4 border-b border-black/10 pb-3">
              <h1 className="font-black uppercase tracking-wider text-base" style={{ color: !isThermal ? accentColor : '#000000' }}>
                {bizName}
              </h1>
              ...
              {showBranchName && (
                <p className="text-[10px] opacity-60 uppercase font-semibold">
                  Chi nhánh: Chi nhánh Trung Tâm (Chính)
                </p>
              )}
            </div>
            ...
            {showHeader && (
              <div className="text-center mb-4">
                <h2 className="text-sm font-black uppercase tracking-widest" style={{ color: !isThermal ? accentColor : '#000000' }}>
                  {headerText}
                </h2>
                <div className="flex justify-between items-center text-[9px] opacity-60 mt-1">
                  <span>Mã đơn: <b className="text-black">{MOCK_ORDER.order_number}</b></span>
                  <span>{new Date(MOCK_ORDER.created_at).toLocaleString('vi-VN', { dateStyle: 'short', timeStyle: 'short' })}</span>
                </div>
              </div>
            )}
              ...
              <tbody className="divide-y divide-black/5">
                {MOCK_ORDER.items.map((item, index) => (
                  <tr key={index} className="align-top">
                    <td className="py-1.5 font-bold pr-1 max-w-[100px] truncate leading-tight">
                      {item.product_name}
                    </td>
