...
export function BarcodePrintDialog({
  ...
  const previewVariant = hasVariants
    ? variants.find((v) => selectedVariants[v.id] && v.barcode) || validVariants[0]
    : null;
      ...
      <DialogContent className="sm:max-w-[400px]">
                  ...
                  {variants.map((v) => (
                    <div key={v.id} className="flex items-center space-x-2">
                      ...
                      >
                        {v.name}{" "}
                        {v.barcode ? (
                          <span className="font-mono text-xs text-muted-foreground ml-1">({v.barcode})</span>
                        ) : (
                          <span className="text-xs text-rose-500 italic ml-1">(Chưa có mã)</span>
                        )}
                      </label>
                    </div>
                  ))}
        ...
        <div style={{ display: "none" }}>
          <div ref={componentRef} className="print-container">
            {hasVariants
              ? variants
                  .filter((v) => selectedVariants[v.id] && v.barcode)
                  .flatMap((v) =>
