  ...
  return (
    <div className="space-y-1.5">
      {paragraphs.map((para, idx) => {
        ...
        if (text.startsWith("###")) {
          return (
            <h3 key={idx} className="text-xs font-black text-foreground mt-2 mb-1">
              {text.replace("###", "").trim()}
            </h3>
          );
        }
        if (text.startsWith("##")) {
          return (
            <h2 key={idx} className="text-sm font-black text-foreground mt-2 mb-1">
              {text.replace("##", "").trim()}
            </h2>
          );
        }
            ...
            <div className="space-y-4">
              {messages.map((m, idx) => (
                <div key={idx} className={`flex gap-2.5 ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                  ...
                  <div
                    className={`rounded-xl p-3 max-w-[85%] border shadow-sm ${
                      m.role === "user"
              ...
              {loading && (
                <div className="flex gap-2.5 justify-start">
                  ...
                  <div className="rounded-xl p-2.5 bg-muted/20 border rounded-tl-none flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-muted-foreground animate-pulse font-bold">
                      AI đang phân tích...
                    </span>
                  </div>
                </div>
