    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Chọn số lượng sản phẩm khách muốn trả lại và số tiền cần hoàn.</DialogDescription>
        </DialogHeader>
                  ...
                  return (
                    <tr key={idx} className={item.returnQty > 0 ? "bg-orange-50/50" : ""}>
                      <td className="p-3 font-semibold text-foreground">
                        {prodName}
                        {variantName && (
                          <span className="block text-xs font-normal text-muted-foreground">{variantName}</span>
                        )}
                        <span className="block text-xs font-medium text-primary mt-1">
                          {formatCurrency(item.unit_price)}/sp
                        </span>
                      </td>
        ...
        <DialogFooter className="border-t pt-4">
          ...
          <Button
            ...
            disabled={!hasItemsToReturn || loading}
