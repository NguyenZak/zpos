        ...
        {loading ? (
          ...
          <div className="grid gap-4 py-4 max-h-[60vh] overflow-y-auto pr-1">
            ...
            <div className="grid gap-3 p-4 bg-muted/30 rounded-xl border">
              <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Thông tin khách hàng</span>
              <div className="grid gap-2">
                <Label htmlFor="cust_name" className="text-xs font-bold">Tên khách hàng</Label>
                <Input
                  id="cust_name"
                  value={customerName}
              ...
              <div className="grid grid-cols-2 gap-2 mt-1">
                <div className="grid gap-2">
                  <Label htmlFor="cust_phone" className="text-xs font-bold">Số điện thoại</Label>
                  <Input
                    id="cust_phone"
                    value={customerPhone}
                ...
                <div className="grid gap-2">
                  <Label htmlFor="cust_addr" className="text-xs font-bold">Địa chỉ giao hàng</Label>
                  <Input
                    id="cust_addr"
                  ...
                  <SelectContent className="max-h-[200px]">
                    {addableItems.map((item) => (
                      <SelectItem key={item.id} value={item.id} className="text-xs font-semibold">
                        {item.name} ({new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(item.price)})
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                    ...
                    {orderItems.map((item, index) => (
                      <div key={index} className="grid grid-cols-12 p-2 items-center hover:bg-muted/5">
                        <div className="col-span-5 font-semibold text-foreground truncate pr-1">
                          ...
                          {item.variant_name && <span className="block text-[10px] font-normal text-muted-foreground mt-0.5">({item.variant_name})</span>}
                        </div>
            ...
            {/* Chi tiết đơn */}
            <div className="grid gap-3 p-4 rounded-xl border bg-background">
              <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Thông tin thanh toán</span>
              
              <div className="grid gap-2">
                <Label htmlFor="total_amount" className="text-xs font-bold">Tổng tiền hóa đơn (VNĐ)</Label>
                <Input
                  id="total_amount"
                  type="number"
                  value={totalAmount}
              ...
              <div className="grid grid-cols-2 gap-3 mt-1">
                <div className="grid gap-2">
                  ...
                  <Select value={status} onValueChange={setStatus}>
                    ...
                    <SelectContent>
                      <SelectItem value="pending" className="text-xs font-semibold">Đang xử lý</SelectItem>
                      <SelectItem value="completed" className="text-xs font-semibold text-green-600">Đã hoàn tất</SelectItem>
                      <SelectItem value="cancelled" className="text-xs font-semibold text-red-600">Đã hủy</SelectItem>
                    </SelectContent>
                  </Select>
                </div>

                <div className="grid gap-2">
                  <Label htmlFor="payment_method" className="text-xs font-bold">Phương thức</Label>
                  <Select value={paymentMethod} onValueChange={setPaymentMethod}>
                    <SelectTrigger id="payment_method" className="h-9 text-xs font-semibold">
                      <SelectValue placeholder="Chọn phương thức" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="cash" className="text-xs font-semibold">Tiền mặt</SelectItem>
                      <SelectItem value="card" className="text-xs font-semibold">Thẻ ngân hàng</SelectItem>
                      <SelectItem value="transfer" className="text-xs font-semibold">Chuyển khoản</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
