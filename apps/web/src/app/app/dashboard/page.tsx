...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const loadTenantName = async () => {
      ...
      try {
        const supabase = createClient();
        const { data: org } = await supabase.from("organizations").select("name").eq("slug", subdomain).maybeSingle();
  ...
  const handleExportReport = () => {
    ...
    try {
      ...
      const csvRows = filteredOrders.map((order: any) => {
        ...
        const method =
          order.payment_method === "cash" ? "Tiền mặt" : order.payment_method === "transfer" ? "Chuyển khoản" : "Thẻ";
  ...
  const getDaysInRange = (start: Date, end: Date) => {
    ...
    while (curr <= finalStop) {
      list.push({
        ...
        name:
          timeRange === "7days" || timeRange === "today"
  ...
  if (stats?.ordersList && Array.isArray(stats.ordersList)) {
    ...
    filteredDebtPayments.forEach((payment: any) => {
      ...
      const revMatch = dynamicRevenueData.find((day) => {
        const d = new Date(day.dateStr);
        return (
          d.getFullYear() === paymentDate.getFullYear() &&
  ...
  const dynamicCategorySalesData =
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
        ...
        <Card className="relative overflow-hidden border-none bg-gradient-to-br from-primary to-primary/80 text-primary-foreground shadow-lg transition-all duration-300 hover:scale-[1.02] hover:shadow-xl">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-xs font-bold tracking-widest uppercase text-primary-foreground/80">
              Doanh thu thực nhận
            </CardTitle>
        ...
        <Card className="border shadow-sm transition-all duration-300 hover:scale-[1.02] bg-card">
          ...
          <CardContent>
            <div className="text-3xl font-extrabold text-amber-600 dark:text-amber-500">{displayDebtInvoiceCount}</div>
                ...
                <div className="h-[300px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={dynamicHourlySalesList} margin={{ top: 15, right: 10, left: 0, bottom: 0 }}>
                      ...
                      <Bar
                        dataKey="sales"
                        name="Doanh thu thực nhận theo giờ"
                ...
                filteredTopProducts.map((p) => (
                  <div key={p.id} className="flex items-center gap-3 border-b pb-3 last:border-0 last:pb-0">
                    <Avatar className="h-9 w-9 rounded-lg border">
                      ...
                      <AvatarFallback className="bg-primary/5 text-primary text-xs rounded-lg">
                        {p.name?.[0]}
                      </AvatarFallback>
                    </Avatar>
                    <div className="flex-1 min-w-0">
                    ...
                    <div className="text-right font-bold text-sm text-primary">{formatCurrency(p.revenue)}</div>
                  </div>
                ))
              ) : (
        ...
        <Card className="border shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <Button
              variant="ghost"
              size="sm"
              className="text-xs font-bold text-primary gap-1 pr-0 hover:bg-transparent"
            >
              Xem tất cả
          ...
          <CardContent>
            <div className="space-y-4">
              {filteredRecentSales.length > 0 ? (
                filteredRecentSales.slice(0, 4).map((item, i) => (
