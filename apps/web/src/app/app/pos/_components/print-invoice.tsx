...
export function PrintInvoice({ order, isProvisional, printType = "payment" }: PrintInvoiceProps) {
  ...
  return (
    <div
      id="print-area"
      className={`hidden print:block print:w-full relative print:bg-white text-black ${fontClass} overflow-hidden`}
    >
      <div className={`${getContainerWidth()} relative z-10`}>
        {/* Accent Bar for A4/A5 */}
        {!isThermal && <div className="w-full h-1.5 mb-6" style={{ backgroundColor: accentColor }} />}
        ...
        <div className="text-center space-y-1 mb-6 border-b border-black/20 pb-4">
          ...
          {bizPhone && <p className={isThermal ? "text-[10px]" : "text-xs"}>SĐT: {bizPhone}</p>}
          {showBranchName && <p className="text-[9px] opacity-75 uppercase font-bold">Chi nhánh: Trung Tâm</p>}
        </div>
        ...
        {showHeader && (
          <div className={`flex ${isThermal ? "flex-col gap-2" : "justify-between items-end"} mb-6`}>
            <div className="space-y-1 text-left">
              ...
              <p className={isThermal ? "text-[10px]" : "text-xs"}>
                Số: <span className="font-bold">{order.order_number}</span>
              </p>
              <p className={isThermal ? "text-[10px]" : "text-xs"}>
            ...
            {showCustomer && (
              <div className={`${isThermal ? "text-left" : "text-right"} space-y-1`}>
                ...
                <p className={`${isThermal ? "text-xs" : "text-sm"} font-black`}>
                  {order.customer?.name || "Khách lẻ"}
                </p>
                {order.customer?.phone && (
                  <p className={isThermal ? "text-[10px]" : "text-xs"}>{order.customer.phone}</p>
                )}
                {showCashierName && <p className="text-[9px] opacity-70">Thu ngân: Admin</p>}
              </div>
            )}
          </div>
        )}
          ...
          <tbody className="divide-y divide-black/10">
            {order.items?.map((item, index) => (
              <tr key={index} className={isThermal ? "text-[10px]" : "text-sm"}>
                ...
                {!isOrderTicket && (
                  <td className="py-2 text-right font-mono">{formatCurrency(item.unit_price || item.price)}</td>
                )}
                {!isOrderTicket && (
          ...
          {!isOrderTicket && showQRCode && bankId && accountNo && (
            <div className="flex flex-col items-center justify-center gap-1.5">
              <div className="border border-black/10 p-1 bg-white inline-block">
                <img src={getQrUrl()} alt="VietQR Transfer" className={isThermal ? "w-24 h-24" : "w-32 h-32"} />
              </div>
