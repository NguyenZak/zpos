...
export default function ConsoleDashboard() {
  ...
  const handleLogout = async () => {
    try {
      ...
      toast.success("Đã đăng xuất khỏi Console!");
      router.push("/login");
    } catch (error) {
  ...
  const syncDatabase = async () => {
    ...
    try {
      ...
      try {
        const [{ data: profs, error: profsError }, membersResult, { data: roleRows }] = await Promise.all([
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          branding: {
            ...
            welcome_message: selectedTenant.welcome_message || "Chúc một ngày kinh doanh thuận lợi, bùng nổ doanh thu.",
          },
        })
    ...
    try {
      ...
      if (data && data[0]) {
        ...
        toast.success("Tạo doanh nghiệp mới thành công!", {
          description: `Subdomain: ${newTenant.slug}.localhost:3000 đã hoạt động.`,
        });
        setTenantViewMode("list");
  ...
  const handleDeleteUser = async (user: SystemUser) => {
    ...
    if (
      !confirm(
        `Xóa nhân viên '${user.full_name}' khỏi tenant hiện tại? Hồ sơ profile cũng sẽ được xóa nếu DB cho phép.`,
      )
    ) {
      return;
    }
                ...
                <SidebarMenuButton size="lg" asChild>
                  <Link href="/console" className="flex items-center gap-3">
                    ...
                    <div className="grid flex-1 text-left text-sm leading-tight">
                      ...
                      <span className="truncate text-[10px] text-muted-foreground font-semibold">SYSTEM CONSOLE</span>
                    </div>
                  </Link>
                </SidebarMenuButton>
              </SidebarMenuItem>
          ...
          <SidebarFooter className="p-4 border-t border-border/80">
            <div className="flex flex-col gap-3">
              ...
              <Button
                ...
                className="w-full justify-start gap-2.5 text-xs text-rose-500 hover:text-rose-500 hover:bg-rose-500/10 dark:hover:bg-rose-500/20 rounded-xl py-5 font-semibold"
              >
              ...
              <div className="flex items-center gap-4">
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <button className="flex items-center gap-2 border-l pl-4 border-border cursor-pointer outline-none hover:opacity-80 transition-opacity">
                    ...
                    <DropdownMenuLabel className="font-normal">
                      <div className="flex flex-col space-y-1">
                        <p className="text-sm font-medium leading-none text-foreground">
                          {currentAdmin?.full_name || "Tài khoản Admin"}
                        </p>
                        <p className="text-xs leading-none text-muted-foreground">{currentAdmin?.email || ""}</p>
                      </div>
                    </DropdownMenuLabel>
                    <DropdownMenuSeparator />
                    <DropdownMenuItem
                      onClick={handleLogout}
                      className="text-rose-500 hover:text-rose-500 focus:text-rose-500 focus:bg-rose-500/10 dark:focus:bg-rose-500/20 cursor-pointer"
                    >
                    ...
                    <CardHeader className="flex flex-row items-center justify-between">
                      <div>
                        <CardTitle className="text-foreground text-base">
                          Tải Lượng & Lưu Lượng API (Real-time)
                        </CardTitle>
            ...
            {activeTab === "tenants" && (
              <div className="space-y-6 animate-fadeIn">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  {tenantViewMode === "list" && (
                    <Button
                      onClick={() => setTenantViewMode("add")}
                      className="button-primary px-4 py-2 gap-2 text-sm shadow-lg shadow-primary/10"
                    >
                  ...
                  <Card className="border-border bg-card/90 shadow-2xl">
                    ...
                    <div className="p-4 border-t border-border flex justify-end gap-3 bg-background/50 rounded-b-xl">
                      <Button
                        ...
                        className="border-border bg-background text-foreground hover:bg-secondary font-bold"
                      >
                        Hủy bỏ
                      </Button>
                      <Button onClick={handleCreateTenant} disabled={isSyncing} className="button-primary px-6 py-2.5">
                        {isSyncing ? "Đang xử lý..." : "Khởi tạo doanh nghiệp"}
                      </Button>
                    </div>
                  </Card>
                )}
                ...
                {tenantViewMode === "edit" && selectedTenant && (
                  <Card className="border-border bg-card/90 shadow-2xl">
                      ...
                      <section className="space-y-4">
                        ...
                        <div className="space-y-1.5">
                          ...
                          <p className="text-[9px] text-muted-foreground mt-1">
                            Không thể thay đổi subdomain để tránh đứt gãy định tuyến.
                          </p>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1.5">
                          ...
                          <div className="space-y-1.5">
                            ...
                            <Select
                              ...
                              onValueChange={(val) =>
                                setSelectedTenant({ ...selectedTenant, subscription_status: val })
                              }
                            >
                              <SelectTrigger className="bg-background border-border text-foreground h-10">
                                <SelectValue />
                    ...
                    <div className="p-4 border-t border-border flex justify-end gap-3 bg-background/50 rounded-b-xl">
                      ...
                      <Button onClick={handleUpdateTenant} disabled={isSyncing} className="button-primary px-6 py-2.5">
              ...
              <div className="space-y-6 animate-fadeIn">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-muted-foreground text-sm mt-1">
                      Dữ liệu lấy từ profiles, organization_members và roles. Console có thể thêm, sửa, gỡ nhân viên
                      khỏi tenant và gán vai trò truy cập.
                    </p>
                  </div>
                  <Dialog open={userFormOpen} onOpenChange={setUserFormOpen}>
                    ...
                    <DialogContent className="max-w-2xl bg-card border-border text-foreground">
                      <DialogHeader>
                        <DialogTitle>
                          {userFormMode === "add" ? "Thêm nhân viên mới" : "Sửa thông tin nhân viên"}
                        </DialogTitle>
                        <DialogDescription>
                          Tạo profile, liên kết tenant và gán vai trò để sidebar/quyền trong app tenant hoạt động theo
                          RBAC.
                        </DialogDescription>
                      </DialogHeader>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-2">
                        ...
                        <div className="space-y-1.5 md:col-span-2">
                          ...
                          <p className="text-[10px] text-muted-foreground">
                            Role RBAC được quản lý trong tenant app tại Cài đặt / Vai trò. Console chỉ gán role cho nhân
                            viên.
                          </p>
                        </div>
                      </div>
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="flex justify-between items-center text-xs">
                        ...
                        <span className="font-mono text-muted-foreground">
                          zpos_backup_2026-05-17_0200.tar.gz (456 MB)
                        </span>
                      </div>
              ...
              <div className="space-y-6 animate-fadeIn text-foreground">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-muted-foreground text-sm mt-1">
                      Theo dõi hành vi người dùng, phát hiện rủi ro brute-force, rò rỉ dữ liệu hoặc đổi quyền bất thường
                      trên toàn bộ tenant.
                    </p>
                  </div>
                ...
                {auditStats.securityAlertsCount > 0 && (
                  <div className="p-4 bg-rose-500/10 border border-rose-500/20 text-rose-200 text-xs rounded-lg flex items-start gap-3 shadow-[0_4px_30px_rgba(244,63,94,0.15)] animate-bounce-subtle">
                    ...
                    <div className="space-y-1 w-full text-left">
                      <p className="font-extrabold uppercase tracking-wider text-[10px] text-rose-400">
                        CẢNH BÁO BẢO MẬT ĐANG HOẠT ĐỘNG ({auditStats.securityAlertsCount})
                      </p>
                      <p className="leading-relaxed font-semibold">
                        Phát hiện hoạt động bất thường hoặc nguy hiểm được ghi nhận bởi Security Engine. Xem các bản ghi
                        màu đỏ bên dưới để phản ứng.
                      </p>
                    </div>
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <Card
                    className={`border-border bg-card/40 shadow-lg backdrop-blur-md transition-all duration-300 ${auditStats.securityAlertsCount > 0 ? "ring-2 ring-rose-500/40 bg-rose-950/10" : ""}`}
                  >
                    <CardContent className="p-5 flex items-center gap-4">
                      <div
                        className={`h-12 w-12 rounded-lg flex items-center justify-center ${auditStats.securityAlertsCount > 0 ? "bg-rose-500/20 border border-rose-500/30" : "bg-muted border border-border"}`}
                      >
                        <ShieldAlert
                          className={`h-5 w-5 ${auditStats.securityAlertsCount > 0 ? "text-rose-400 animate-pulse" : "text-muted-foreground"}`}
                        />
                      </div>
                      <div className="space-y-0.5">
                        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">
                        ...
                        <h3
                          className={`text-2xl font-black ${auditStats.securityAlertsCount > 0 ? "text-rose-400" : "text-foreground"}`}
                        >
                  ...
                  <Card
                    className={`border-border bg-card/40 shadow-lg backdrop-blur-md ${auditStats.failedLoginsCount >= 3 ? "ring-1 ring-amber-500/30 bg-amber-950/10" : ""}`}
                  >
                    <CardContent className="p-5 flex items-center gap-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        {auditStats.topRiskyTenants.length === 0 ? (
                          ...
                          auditStats.topRiskyTenants.map((item: any, idx: number) => {
                            ...
                            return (
                              <div
                                key={item.slug}
                                className="p-4 flex items-center justify-between gap-4 hover:bg-muted/20 transition-all duration-200"
                              >
                                <div className="min-w-0 flex-1 space-y-1 text-left">
                                  <div className="flex items-center gap-2">
                                    <span className="text-xs font-bold text-foreground capitalize">{item.slug}</span>
                                    <Badge
                                      className={`text-[8px] font-extrabold uppercase scale-90 px-1 py-0 ${
                                        item.score >= 10
                  ...
                  <Card className="border-border bg-card/60 shadow-lg lg:col-span-2">
                    <CardHeader className="pb-3 border-b border-border/40">
                      ...
                      <CardDescription className="text-muted-foreground text-xs">
                        Kích hoạt các kịch bản tấn công/sơ suất nghiệp vụ để kiểm thử phản ứng của Security Engine tức
                        thì.
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="p-5 space-y-6">
                      ...
                      <div className="space-y-2">
                        <p className="text-[10px] font-bold text-rose-400 uppercase tracking-wide text-left">
                          1. Giả lập Các Kịch Bản Rủi Ro & Độc Hại
                        </p>
                      ...
                      <div className="p-4 rounded-xl bg-slate-900/60 border border-border flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-left">
                        <div className="space-y-1">
                          <p className="text-[10px] font-bold text-teal-400 uppercase tracking-wide">
                            2. Quản Lý Chính Sách Lưu Trữ Log (Retention Policy)
                          </p>
                          <p className="text-[11px] text-muted-foreground">
                            Thời hạn tự dọn dẹp: <strong className="text-white">INFO</strong> 30 ngày |{" "}
                            <strong className="text-white">WARNING</strong> 90 ngày |{" "}
                            <strong className="text-white">ERROR</strong> 180 ngày |{" "}
                            <strong className="text-white">CRITICAL</strong> 365 ngày.
                          </p>
                        </div>
                        <Button
                      ...
                      <Table className="border-collapse w-full">
                        <TableHeader className="bg-slate-900/60 sticky top-0 z-10 border-b border-border/80 text-xs">
                          <TableRow>
                            ...
                            <TableHead className="font-extrabold text-foreground px-4 py-3.5 text-left w-36">
                              Thời gian
                            </TableHead>
                            <TableHead className="font-extrabold text-foreground px-4 py-3.5 text-left w-24">
                              Tenant
                            </TableHead>
                            <TableHead className="font-extrabold text-foreground px-4 py-3.5 text-left w-48">
                              Thành viên
                            </TableHead>
                            <TableHead className="font-extrabold text-foreground px-4 py-3.5 text-left w-36">
                              Module / Action
                            </TableHead>
                            <TableHead className="font-extrabold text-foreground px-4 py-3.5 text-left w-28">
                              Mức độ
                            </TableHead>
                            <TableHead className="font-extrabold text-foreground px-4 py-3.5 text-left min-w-[200px]">
                              Thông tin
                            </TableHead>
                            <TableHead className="font-extrabold text-foreground px-4 py-3.5 text-left w-32">
                              Địa chỉ IP
                            </TableHead>
                          </TableRow>
                        </TableHeader>
                                ...
                                <React.Fragment key={log.id}>
                                  ...
                                  >
                                    ...
                                    <TableCell className="p-3 space-y-0.5 text-left">
                                      ...
                                      <p className="font-bold text-foreground tracking-tight text-[11px] whitespace-nowrap truncate">
                                        {log.action}
                                      </p>
                                    </TableCell>
                                    ...
                                    <TableCell className="p-3 shrink-0">
                                      <Badge
                                        className={`text-[9px] font-extrabold uppercase scale-90 px-2 py-0.5 border ${
                                        ...
                                        <div className="space-y-4 animate-fadeIn">
                                          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-border/30 pb-2">
                                            <div className="space-y-1">
                                              <p className="text-xs font-bold text-foreground">
                                                Chi Tiết Log Mật Khóa - ID: {log.id}
                                              </p>
                                              <p className="text-[10px] text-muted-foreground font-mono">
                                                User Agent: {log.user_agent || "Không thu nhận"}
                                              </p>
                                            </div>
                                            {log.is_alert && (
                                          ...
                                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            ...
                                            <div className="md:col-span-1 space-y-2 text-xs">
                                              ...
                                              {log.is_alert && (
                                                <div className="p-3.5 rounded-xl border border-rose-500/20 bg-rose-500/5 text-rose-300">
                                                  <p className="font-bold uppercase tracking-wider text-[9px] text-rose-400">
                                            ...
                                            <div className="md:col-span-2 space-y-1">
                                              <p className="text-[10px] font-bold text-teal-400 uppercase tracking-wide">
                                                Trình Giám Sát Metadata Payload (Đã lọc bỏ thông tin nhạy cảm 🛡️)
                                              </p>
