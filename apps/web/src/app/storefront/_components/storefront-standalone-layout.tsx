...
import { CMSLoginForm } from "@/app/cms/_components/cms-login-form";

interface StorefrontStandaloneLayoutProps {
...
export function StorefrontStandaloneLayout({ children, variant, collapsible, users }: StorefrontStandaloneLayoutProps) {
  ...
  return (
    <SidebarProvider defaultOpen={false} style={{ "--sidebar-width": "0px" } as React.CSSProperties}>
      <SidebarInset className="[html[data-content-layout=centered]_&>*]:mx-auto [html[data-content-layout=centered]_&>*]:w-full [html[data-content-layout=centered]_&>*]:max-w-screen-2xl">
        <header className="flex h-12 shrink-0 items-center gap-2 border-b transition-[width,height] ease-linear [html[data-navbar-style=sticky]_&]:sticky [html[data-navbar-style=sticky]_&]:top-0 [html[data-navbar-style=sticky]_&]:z-50 [html[data-navbar-style=sticky]_&]:overflow-hidden [html[data-navbar-style=sticky]_&]:rounded-t-[inherit] [html[data-navbar-style=sticky]_&]:bg-background/50 [html[data-navbar-style=sticky]_&]:backdrop-blur-md">
          <div className="flex w-full items-center justify-between px-4 lg:px-6">
            <div className="flex items-center gap-1 lg:gap-2">
              <div className="flex items-center gap-2 mr-2">
                <div className="h-6 w-6 rounded bg-primary flex items-center justify-center">
                  <span className="font-bold text-primary-foreground text-xs">SF</span>
                </div>
                <span className="font-semibold text-sm hidden md:inline-flex tracking-tight">CMS Storefront</span>
              </div>
        ...
        <div className="h-full p-3 md:p-4 print:p-0">{children}</div>
      </SidebarInset>
    </SidebarProvider>
  );
}
